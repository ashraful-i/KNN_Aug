...
if __name__ == '__main__':
    ...
    for x in range(split_times):
        ...
        _list1.append(s_l)
        _list1.append(d_l)'''
        div_all = pd.concat([s_l, d_l])
        #div_all = pd.DataFrame(_list1)


        get_grp_point(test_set, len(div_all), div_all, True)
        #print(divided_grps)
        d_grp = divided_grps
        _df_lst_1 = []
        ...
        for cnt, d_f_plt in enumerate(_df_lst_1):
            #print(d_f_plt)
            d_f_plt.to_csv("ans_"+str(cnt)+".csv")
            '''
            d_f_plt.hist(column=['age', 'DEATH_EVENT'])
            d_f_plt.hist(column=['serum_creatinine', 'DEATH_EVENT'])
            plt.show()'''
